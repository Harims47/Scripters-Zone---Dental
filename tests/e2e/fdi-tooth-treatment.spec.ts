...
test.describe('DentalCore v1.1 — FDI Interactive Tooth Treatment Planning Suite (PostgreSQL)', () => {
  ...
  test('FDI-03: Single tooth treatment saves toothNumber: 16 independently in PostgreSQL and updates planned list', async ({ page }) => {
    await loginAs(page, 'dutyDoctor');
    const target = await getActiveTestPatient();
    ...
    await firstProcedure.click({ force: true });
    ...
    // Direct PostgreSQL Read-Only Verification
    const savedItem = await queryOne<{ toothNumber: number; notes: string }>(`
      ...
      WHERE tp."patientId" = $1 AND tpi.notes = $2
      LIMIT 1
  ...
  test('FDI-04: Multi-tooth selection supports removal and batch creates separate items per tooth', async ({ page }) => {
    ...
    // Verify in PostgreSQL that 3 distinct items were created
  ...
  test('FDI-05: Non-tooth general treatment persists with toothNumber = NULL and displays General badge', async ({ page }) => {
    ...
    // Verify PostgreSQL has toothNumber NULL
  ...
  test('FDI-06: Editing a planned treatment modifies tooth assignment and notes cleanly in PostgreSQL', async ({ page }) => {
    await loginAs(page, 'dutyDoctor');
    const target = await getActiveTestPatient();
  ...
  test('FDI-07: Deleting a planned treatment removes the item from UI and PostgreSQL', async ({ page }) => {
    await loginAs(page, 'dutyDoctor');
    const target = await getActiveTestPatient();
    ...
    const countBefore = await queryOne<{ total: string | number }>(`
      SELECT COUNT(*) as total FROM "TreatmentPlanItem" tpi
      JOIN "TreatmentPlan" tp ON tpi."treatmentPlanId" = tp.id
      WHERE tp."patientId" = $1
    `, [target.id]);
    ...
    if (await deleteBtn.isVisible()) {
      ...
      const countAfter = await queryOne<{ total: string | number }>(`
        SELECT COUNT(*) as total FROM "TreatmentPlanItem" tpi
        JOIN "TreatmentPlan" tp ON tpi."treatmentPlanId" = tp.id
        WHERE tp."patientId" = $1
      `, [target.id]);

      expect(Number(countAfter!.total)).toBeLessThan(Number(countBefore!.total));
    }
